...
description: 以「白銀礦業價格 ÷ 白銀價格」的相對比率衡量礦業板塊相對於金屬本體的估值區間（偏貴/偏便宜），並用歷史分位數與類比區間推導「底部/頂部」訊號與情境推演。
---
...
其中：
- **miner_price**：白銀礦業代表（ETF 如 SIL/SILJ，或自建礦業指數）
- **metal_price**：白銀價格（期貨 SI=F、現貨 XAGUSD、ETF SLV）

此比率衡量「礦業相對於金屬本體」的估值水位：
- **比率高**：礦業相對白銀偏貴（可能過度樂觀、槓桿溢價高）
- **比率低**：礦業相對白銀偏便宜（可能被低估、或反映成本/股權稀釋風險）
</principle>
...
| 分位數區間 | 標籤           | 直覺                     |
|------------|----------------|--------------------------|
| ≤ 20%      | bottom (底部)  | 礦業相對白銀歷史上很便宜 |
| 20-40%     | low (偏低)     | 礦業相對估值偏低         |
| 40-60%     | neutral (中性) | 歷史中位區間             |
| 60-80%     | high (偏高)    | 礦業相對估值偏高         |
| ≥ 80%      | top (頂部)     | 礦業相對白銀歷史上很貴   |

**底部區間不等於白銀必漲**：可能是礦業因成本/稀釋被合理定價。
</principle>
...
1. **數據整合**：取得礦業代理與白銀價格序列
2. **比率計算**：計算相對比率並可選平滑
3. **分位數判斷**：當前比率在歷史的位置
4. **類比事件**：歷史底部區間的事件識別
5. **前瞻驗證**：底部事件後白銀的 1/2/3 年表現
6. **情境推演**：礦業需漲多少 / 白銀需跌多少才回到頂部
...
pip install pandas numpy yfinance  # 首次使用
...
2. **完整分析** - 自訂參數進行情境分析（可選擇礦業/金屬代理、分位門檻）
3. **歷史驗證** - 查看底部區間事件的前瞻報酬統計
4. **情境推演** - 計算「回到頂部」需要的礦業漲幅或白銀跌幅
5. **方法論學習** - 了解比率邏輯與分位數解讀
...
| 3, "歷史", "驗證", "backtest" | 閱讀 `workflows/analyze.md` 並聚焦歷史類比      |
| 4, "情境", "scenario", "推演" | 閱讀 `workflows/analyze.md` 並聚焦情境推演      |
| 5, "學習", "方法論", "why"    | 閱讀 `references/methodology.md`                |
| 提供參數 (如礦業/金屬代理)    | 閱讀 `workflows/analyze.md` 並使用參數執行      |
...
| data-research.md | 數據源研究   | 了解如何獲取或替代礦業/金屬數據 |
</workflows_index>
...
| ratio_analyzer.py | `--miner-proxy SILJ --freq 1mo`      | 自訂礦業與頻率    |
...
| 參數        | 類型   | 預設值  | 說明                          |
|-------------|--------|---------|-------------------------------|
| miner_proxy | string | SIL     | 白銀礦業代表（ETF/指數代號）  |
...
| forward_horizons    | list   | [252, 504, 756] | 前瞻期（交易日，對應 1/2/3 年）  |
...
```json
{
  ...
  "summary": "白銀礦業/白銀比率落在歷史低分位，顯示礦業相對白銀偏便宜...",
  "notes": [
    "比率訊號衡量的是『相對估值』，不是單邊價格保證。",
    "礦業與金屬可能同漲，但礦業也可能因成本上升、地緣/政策風險、增發稀釋而落後。",
    "建議搭配：礦業獲利率(成本曲線)、白銀實質利率/美元、投機部位(COT)、ETF 流量等做交叉驗證。"
  ]
}
```
